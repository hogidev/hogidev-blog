...
export function buildModel(): THREE.Group {
  ...
  const M = {
    ...
    screen: new THREE.MeshStandardMaterial({
      name: 'screen',
      color: '#10241c',
      emissive: new THREE.Color('#2fbf82'),
  ...
  stairs.name = 'stairs';
  g.add(stairs);
  const stepRise = 0.146, stepRun = 0.11, stepCount = 10;
  const treadRun = 0.12, treadDepth = 0.45, treadThickness = 0.035;
  const stairX0 = 0.555, stairFloorY = 0.14, stairZ = -0.955;
  // angled stringer braces the treads from underneath, like a real staircase
  const dxTotal = (stepCount - 1) * stepRun;
  const dyTotal = stepCount * stepRise;
  const slopeAngle = Math.atan2(dyTotal, dxTotal);
  const midX = stairX0 + dxTotal / 2;
  const midY = stairFloorY + dyTotal / 2;
  const gap = 0.085;
  const stringer = box(
    'stair_stringer', M.woodDk,
    Math.hypot(dxTotal, dyTotal) + 0.14, 0.05, treadDepth - 0.05,
    midX + Math.sin(slopeAngle) * gap, midY - Math.cos(slopeAngle) * gap, stairZ,
    stairs
  );
  stringer.rotation.z = slopeAngle;
  for (let i = 0; i < stepCount; i++) {
    const x = stairX0 + i * stepRun;
    const topY = stairFloorY + (i + 1) * stepRise;
    box(`step_${i}`, M.wood, treadRun, treadThickness, treadDepth, x, topY - treadThickness / 2, stairZ, stairs);
    rbox(`step_nose_${i}`, M.woodDk, treadRun + 0.005, 0.03, treadDepth + 0.01, 0.012, x, topY - 0.012, stairZ, stairs);
  }
  const rail = new THREE.Group();
  rail.name = 'stair_rail';
  g.add(rail);
  const railPostIdx = [0, 3, 6, 9];
  const postTopY = (i: number) => stairFloorY + (i + 1) * stepRise + 0.62;
  const postX = (i: number) => stairX0 + i * stepRun;
  railPostIdx.forEach((i) => {
    const topY = stairFloorY + (i + 1) * stepRise;
    cyl(`rail_post_${i}`, M.dark, 0.014, 0.014, 0.62, postX(i), topY + 0.31, -0.72, rail, 12);
  });
  // handrail spans the first-to-last post exactly, plus a small overhang on each end
  const hrFirst = railPostIdx[0], hrLast = railPostIdx[railPostIdx.length - 1];
  const hrOverhang = 0.08;
  const hrDx = postX(hrLast) - postX(hrFirst), hrDy = postTopY(hrLast) - postTopY(hrFirst);
  const hrAngle = Math.atan2(hrDy, hrDx);
  const hrLen = Math.hypot(hrDx, hrDy) + hrOverhang * 2;
  const hr = rbox('handrail', M.wood, hrLen, 0.05, 0.055, 0.02,
    (postX(hrFirst) + postX(hrLast)) / 2, (postTopY(hrFirst) + postTopY(hrLast)) / 2, -0.72, rail);
  hr.rotation.z = hrAngle;
  ...
  const p1 = rbox('sofa_pillow_1', M.mint, 0.28, 0.28, 0.11, 0.05, -0.4, 0.58, 0.13, sofa);
  p1.rotation.z = 0.15;
  const p2 = rbox('sofa_pillow_2', M.coral, 0.28, 0.28, 0.11, 0.05, 0.42, 0.58, 0.13, sofa);
  p2.rotation.z = -0.1;
  ...
  table.position.set(-0.6, F1, -0.05);
  g.add(table);
  ...
  cons.name = 'tv_console';
  cons.position.set(-0.6, F1, -1.05);
  g.add(cons);
  ...
  const flamp = new THREE.Group();
  flamp.name = 'floor_lamp';
  flamp.position.set(-1.5, F1, 0.95);
  g.add(flamp);
  ...
  const bp2 = rbox('bed_pillow_2', M.mint, 0.3, 0.1, 0.22, 0.04, 0.1, 0.42, -0.5, bed);
  bp2.rotation.z = 0.12;
  ...
  const bside = new THREE.Group();
  bside.name = 'bedside';
  bside.position.set(-1.5, F2, 0.55);
  g.add(bside);
  ...
  desk.name = 'desk';
  desk.position.set(0.35, F2, -0.98);
  g.add(desk);
  ...
  const mp = rbox('monitor_panel', M.dark, 0.5, 0.3, 0.035, 0.01, -0.16, 1.02, -0.12, desk);
  mp.rotation.y = 0.12;
  const ms = box('monitor_screen', M.screen, 0.46, 0.26, 0.012, -0.16, 1.02, -0.098, desk);
  ms.rotation.y = 0.12;
  cyl('monitor2_stand', M.metal, 0.025, 0.045, 0.11, 0.28, 0.79, -0.1, desk, 14);
  const mp2 = rbox('monitor2_panel', M.dark, 0.3, 0.4, 0.035, 0.01, 0.28, 1.04, -0.12, desk);
  mp2.rotation.y = -0.15;
  const ms2 = box('monitor2_screen', M.screen, 0.26, 0.36, 0.012, 0.28, 1.04, -0.098, desk);
  ms2.rotation.y = -0.15;
  ...
  chair.name = 'chair';
  chair.position.set(0.22, F2, -0.42);
  g.add(chair);
  ...
  const cb = rbox('chair_back', M.coral, 0.36, 0.48, 0.08, 0.035, 0, 0.63, 0.19, chair);
  cb.rotation.x = -0.12;
  ...
  hairCap.position.y = 0.79;
  hairCap.scale.set(0.96, 1.05, 0.96);
  boy.add(hairCap);
